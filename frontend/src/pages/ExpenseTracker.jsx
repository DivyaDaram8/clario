...
const ExpenseTracker = () => {
  ...
  // Reusable Input Component for glass effect
  const GlassInput = ({ label, value, onChange, type = "text", placeholder }) => (
    <div>
      ...
      <input
        ...
        className="w-full px-3 py-2 rounded-xl bg-white/30 backdrop-blur-md border border-white/40 focus:outline-none focus:ring-2 focus:ring-blue-500 placeholder-gray-400 text-gray-800 shadow-md transition"
      />
    </div>
  );
  ...
  const GlassSelect = ({ label, children }) => (
    <div>
      <label className="block mb-1 font-medium text-gray-700">{label}</label>
      <select className="w-full px-3 py-2 rounded-xl bg-white/30 backdrop-blur-md border border-white/40 focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-800 shadow-md transition">
        {children}
      </select>
    </div>
  );
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
            ...
            <div className="bg-gray-50 rounded-xl shadow-md p-5 flex flex-col justify-center items-center hover:shadow-lg transition">
              ...
              <p className="text-3xl md:text-4xl font-bold text-green-600">${totalBalance.toLocaleString()}</p>
            </div>
            ...
            <div className="bg-gray-50 rounded-xl shadow-md p-5 flex flex-col justify-between hover:shadow-lg transition relative">
              <p className="text-gray-600 italic text-base md:text-lg text-center mb-4">"{moneyQuote}"</p>
              <button
                onClick={handleAddTransactionClick}
                className="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-5 rounded-lg flex items-center justify-center space-x-2 text-base md:text-lg"
              >
                <FaPlus className="text-sm md:text-base" />
                <span>Add Transaction</span>
              </button>

              {/* Expense/Income Options */}
              {showOptions && (
                <div className="absolute top-16 left-1/2 transform -translate-x-1/2 flex space-x-6 z-30">
                  <button
                    onClick={() => handleTypeClick("Expense")}
                    className="flex flex-col items-center justify-center w-16 h-16 rounded-full bg-red-600 text-white hover:bg-red-700 transition"
                  >
                  ...
                  <button
                    ...
                    className="flex flex-col items-center justify-center w-16 h-16 rounded-full bg-green-600 text-white hover:bg-green-700 transition"
                  >
            ...
            <div className="bg-gray-50 rounded-xl shadow-md p-5 hover:shadow-lg transition relative">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg md:text-xl font-semibold text-gray-800">Spending by Category</h2>
                <button
              ...
              <div className="space-y-3">
                {categories.map((cat, index) => {
                  const Icon = cat.icon;
                  return (
                    <div key={index} className="flex items-center justify-between text-base md:text-lg">
                      <div className="flex items-center space-x-3">
                        <Icon className={`${cat.color} text-lg md:text-xl`} />
  ...
  {COLORS.map((color, idx) => (
    ...
    ></div>
                    ...
                    {ICONS.map((IconComponent, idx) => (
                      <div
                        ...
                        className={`p-2 cursor-pointer rounded-lg flex items-center justify-center border-2 ${
                          newCategory.icon === IconComponent ? "border-black" : "border-transparent"
          ...
          <div className="bg-gray-50 rounded-xl shadow-md p-5 hover:shadow-lg transition h-[450px] overflow-y-auto">
            <h2 className="text-xl md:text-2xl font-semibold text-gray-800 mb-4">Transaction History</h2>
                ...
                {transactions.map((t) => (
                  <tr key={t.id} className="border-b border-gray-100 hover:bg-gray-100 transition">
                    ...
                    <td className="py-2 px-3">
                      <span className={`px-3 py-1 rounded-full text-xs md:text-sm font-medium ${t.type === "Income" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}>
                        {t.type}
                      </span>
                    </td>
