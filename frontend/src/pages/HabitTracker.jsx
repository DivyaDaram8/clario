...
const HabitTracker = () => {
 ...
 useEffect(() => {
   fetchHabits();
   // ensure currentDate is a date-only (hours=0) local
   const d = new Date();
   d.setHours(0, 0, 0, 0);
   setCurrentDate(d);
 }, []);

 // --- Helpers for date normalization to avoid timezone shift issues ---
 ...
 // Create a Date object set to local midnight for a given date (Date | string)
 ...
 // Toggle habit completion — send normalized YYYY-MM-DD string to backend to avoid timezone shifts
 const toggleHabitLog = async (habitId, date) => {
   try {
     const dateStr = toLocalDateString(date); // normalized date string
     ...
     // If modal is open for this habit and month, refresh monthly stats
 ...
 // Open month modal and fetch stats for current month
 ...
 const getWeekDays = (reference = new Date()) => {
   // Return an array of 7 Date objects for the week containing 'reference'
   const today = new Date(reference);
   today.setHours(0, 0, 0, 0);

   const currentDay = today.getDay();
   // We want Monday to Sunday display. JS getDay() -> 0 Sun, 1 Mon ... 6 Sat
 ...
 // Check whether a habit is completed for a given date (date is Date object)
 ...
 // Allow checking if date is <= today (local date-only)
 const canCheckDate = (date) => {
   const d = toLocalMidnight(date);
   const today = toLocalMidnight(new Date());
   // allow past and today
 ...
 const getMonthCalendar = () => {
   ...
   // Add empty cells for days before the first day of the month (Sun=0 ... Sat=6)
                     ...
                     <div
                       ...
                       onClick={async () => {
                         if (dayData && dayData.canCheck) {
                           ...
                           // refresh monthly stats after toggle
